...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* ─── Connections & Integrations (top priority) ─────────────────── */}
...
function ConnectedAccountSection() {
  ...
  if (!connected) {
    return (
      <div className="relative overflow-hidden bg-gradient-to-r from-amber-50 via-orange-50 to-amber-50 border-2 border-amber-300/70 rounded-xl p-6">
        <div className="absolute top-0 right-0 w-32 h-32 bg-amber-200/30 rounded-full -translate-y-1/2 translate-x-1/2" />
        <div className="absolute bottom-0 left-8 w-16 h-16 bg-orange-200/30 rounded-full translate-y-1/2" />
        <div className="relative">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-amber-100 border border-amber-300 rounded-xl flex items-center justify-center shrink-0">
              <AlertCircle className="w-6 h-6 text-amber-600" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-lg text-amber-900">Complete Your Setup</h2>
                <span className="text-[10px] font-bold uppercase tracking-wider bg-amber-200 text-amber-800 px-2 py-0.5 rounded-full">Action Required</span>
              </div>
              <p className="text-sm text-amber-700/90 mt-1.5 leading-relaxed">
                You haven&apos;t connected any ad platforms yet. Link your Google Ads account and other platforms
                to unlock AI-powered campaign optimization, smart budget management, and cross-channel intelligence.
              </p>
              <div className="flex flex-wrap items-center gap-3 mt-4">
                <Link
                  href="/onboarding"
                  className="inline-flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition shadow-sm"
                >
                  <Zap className="w-4 h-4" />
                  Complete Onboarding
                  <ChevronRight className="w-4 h-4" />
                </Link>
                <span className="text-xs text-amber-600 font-medium">Takes less than 2 minutes</span>
              </div>
            </div>
          </div>
        </div>
