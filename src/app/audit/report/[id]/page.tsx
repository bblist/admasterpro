...
function StatusBadge({ status }: { status: string }) {
    ...
    const config: Record<string, { bg: string; text: string; icon: React.ReactNode; label: string }> = {
        excellent: { bg: "bg-green-500/10 border-green-500/30", text: "text-green-400", icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: t("auditReport.excellent") },
        good: { bg: "bg-blue-500/10 border-blue-500/30", text: "text-blue-400", icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: t("auditReport.good") },
        "needs-work": { bg: "bg-amber-500/10 border-amber-500/30", text: "text-amber-400", icon: <AlertTriangle className="w-3.5 h-3.5" />, label: t("auditReport.needsWork") },
        critical: { bg: "bg-red-500/10 border-red-500/30", text: "text-red-400", icon: <XCircle className="w-3.5 h-3.5" />, label: t("auditReport.critical") },
    };
...
export default function AuditReportPage() {
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-muted">{t("auditReport.loading")}</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center max-w-md">
                    ...
                    <h2 className="text-xl font-bold text-foreground mb-2">{t("auditReport.notFound")}</h2>
                    <p className="text-muted mb-6">{error || t("auditReport.notFoundDesc")}</p>
                    <Link href="/audit" className="inline-flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-lg font-medium hover:bg-primary-dark transition">
                        {t("auditReport.startNew")} <ArrowRight className="w-4 h-4" />
                    </Link>
                </div>
            </div>
        );
    ...
    const { result } = data;
    const reportDate = data.createdAt ? new Date(data.createdAt).toLocaleDateString(locale, { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" }) : new Date().toLocaleDateString(locale, { year: "numeric", month: "long", day: "numeric" });
            ...
            <div className="min-h-screen bg-background">
                    ...
                    <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <Link href="/onboarding" className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark transition">
                                {t("auditReport.continueSetup")} <ArrowRight className="w-4 h-4" />
                            </Link>
                            <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-card border border-border text-foreground rounded-lg text-sm font-medium hover:border-primary/30 transition">
                                <Printer className="w-4 h-4" /> {t("auditReport.printPdf")}
                            </button>
                ...
                <div className="max-w-5xl mx-auto px-4 py-8">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <span className="text-sm font-medium text-primary">{t("auditReport.reportLabel")}</span>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3 mt-3 text-xs text-muted">
                                    ...
                                    {data.monthlySpend && <span className="bg-card border border-border px-2 py-1 rounded">{t("auditReport.adSpend", { spend: data.monthlySpend })}</span>}
                                    <span className="bg-card border border-border px-2 py-1 rounded">{reportDate}</span>
                                </div>
                            </div>
                            <div className="text-center">
                                <ScoreRing score={result.overallScore} />
                                <div className="text-xs text-muted mt-2">{t("auditReport.overallScore")}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    {result.sections && result.sections.map((section, i) => (
                        <div key={i} className={`bg-card border border-border rounded-xl p-6 mb-6 ${i === 4 ? "print-break" : ""}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-5">
                                {/* Findings */}
                                <div>
                                    <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-3">{t("auditReport.findings")}</h4>
                                ...
                                {/* Recommendations */}
                                <div>
                                    <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-3">{t("auditReport.recommendations")}</h4>
                                    <div className="space-y-2">
                    ...
                    {result.competitorInsight && result.competitorInsight !== "N/A" && (
                        <div className="bg-card border border-border rounded-xl p-6 mb-8">
                            <h2 className="text-lg font-semibold text-foreground mb-3">
                                {t("auditReport.competitiveInsight")}
                            </h2>
                    ...
                    <div className="bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 rounded-2xl p-8 text-center mb-8 no-print">
                        ...
                        <p className="text-muted mb-6 max-w-lg mx-auto">
                            {t("auditReport.ctaDesc")}
                        </p>
                        <div className="flex flex-col sm:flex-row gap-3 justify-center">
                            <Link href="/dashboard/chat" className="inline-flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-6 py-3 rounded-lg font-semibold transition">
                                {t("auditReport.chatWithAi")} <ArrowRight className="w-4 h-4" />
                            </Link>
                            <Link href="/dashboard/knowledge-base" className="inline-flex items-center gap-2 bg-card border border-border text-foreground px-6 py-3 rounded-lg font-medium hover:border-primary/30 transition">
                                {t("auditReport.viewKb")}
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="border-t border-border pt-6 pb-12">
                        <div className="flex items-center justify-between text-xs text-muted">
                            <div className="flex items-center gap-2">
                                ...
                                <span>{t("auditReport.generatedBy")}</span>
                            </div>
                            <div>{reportDate}</div>
                        </div>
                        <div className="text-[10px] text-muted/50 mt-2">
                            {t("auditReport.disclaimer")}
                            {" "}{t("auditReport.reportId", { id: auditId })}
                        </div>
                    </div>
                </div>
            </div>
